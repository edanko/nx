...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withLaunches; query != nil {
		...
		for _, n := range neighbors {
			fk := n.order_launches
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "order_launches" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "order_launches" returned %v for node %v`, *fk, n.ID)
			}
